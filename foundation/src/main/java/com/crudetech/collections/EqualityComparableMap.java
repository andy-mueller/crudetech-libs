 ...
 * instead of the build in {@link Object#hashCode()} and {@link Object#equals(Object)} methods.
 * <p/>
 ...
 * };
 *
 * Map&lt;Integer, String&gt; anotherMap = EqualityComparableMap.newHashMap(unsignedComparer);
 * </pre>
 * <p/>
 *
...
public class EqualityComparableMap<TKey, TValue> implements Map<TKey, TValue> {
    ...
    /**
     * Creates a new Map that embodies a {@link LinkedHashMap}.
     *
     * @param equComp           The comparer to be used for all equal and hash functionality.
     * @param removeOldestEntry A function that is called to determine if the oldest entry shall be removed
     *                          when a new entry is added. Checkout {@link LinkedHashMap#removeEldestEntry(java.util.Map.Entry)}
     *                          for more details.
     * @param <K>
     * @param <V>
     * @return A ready to use Map.
     */
    ...
    public static <K, V> Map<K, V> newLinkedHashMap(EqualityComparer<K> equComp, final UnaryFunction<Entry<K, V>, Boolean> removeOldestEntry) {
        return new EqualityComparableMap<K, V>(equComp,
                new LinkedHashMap<EqualityComparable<K>, V>() {
                    @Override
